...
def run_model_eval() -> None:
    for case in EVAL_QUERIES:
        ...
        variations = generate_query_variations(case["query"], ["Baseline", "Macro regime", "Risk and counterargument"])
        harness = run_variation_harness(config, variations["Context"], case["expected_terms"])
        assert len(harness) == 3, f"{case['query']}: variation harness should produce one row per context"
        assert harness["Quality score"].between(0, 100).all(), f"{case['query']}: quality score out of range"
